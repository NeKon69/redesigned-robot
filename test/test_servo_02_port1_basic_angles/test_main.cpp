...
void test_port1_direction_and_stop() {
  servo1.attach(SERVO_PORT1_PIN);
  TEST_ASSERT_TRUE_MESSAGE(servo1.attached(), "Servo on port 1 did not attach.");

  Serial.println("Port1 continuous-servo direction check.");
...
void setup() {
  ...
  UNITY_BEGIN();
  RUN_TEST(test_port1_direction_and_stop);
  UNITY_END();
}
